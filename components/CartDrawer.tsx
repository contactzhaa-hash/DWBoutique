...
export default function CartDrawer() {
  const { cart, isCartOpen, setIsCartOpen, removeFromCart, updateQuantity, formatPrice } = useShop();
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md bg-[#FBF9F5] h-full shadow-2xl z-10 flex flex-col justify-between border-l border-black/[0.08]"
          >
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              {cart.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  >
                    Explore Current Runway Collections
                  </button>
                </div>
              ) : (
                cart.map((item) => (
                  <div
                    ...
                    className="flex space-x-4 pb-6 border-b border-black/[0.04]"
                  >
                    <img
            ...
            {cart.length > 0 && (
              <div className="p-6 border-t border-black/[0.08] bg-[#F5F2EC] space-y-4">
                ...
                <p className="text-[10px] text-[#737373]">
                  Complimentary worldwide insured courier shipping on all bespoke couture orders.
                </p>
                <button
                  onClick={() => alert('Proceeding to Luxury Encrypted Checkout Engine')}
                  className="w-full py-4 bg-[#111111] text-white text-xs uppercase tracking-[0.25em] font-medium hover:bg-[#C5A059] transition-colors flex items-center justify-center space-x-2 rounded-none"
                >
